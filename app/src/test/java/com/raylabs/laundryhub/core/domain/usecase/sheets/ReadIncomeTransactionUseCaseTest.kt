...
class ReadIncomeTransactionUseCaseTest {
    ...
    fun `returns success when repository returns success`() = runTest {
        ...
        val result = useCase.invoke(filter = FILTER.SHOW_ALL_DATA, rangeDate = null)
    ...
    fun `returns error when repository returns null`() = runTest {
        whenever(repository.readIncomeTransaction(FILTER.SHOW_ALL_DATA, null)).thenReturn(null)
        val result = useCase.invoke(filter = FILTER.SHOW_ALL_DATA, rangeDate = null)
