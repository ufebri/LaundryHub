...
class DeleteOutcomeUseCaseTest {
    ...
    fun `invoke returns success when repository delete succeeds`() = runTest {
        whenever(repository.deleteOutcome("OUT123")).thenReturn(Resource.Success(true))

        val result = deleteOutcomeUseCase(outcomeId = "OUT123")

        assertEquals(Resource.Success(true), result)
        verify(repository).deleteOutcome("OUT123")
    }

    @Test
    fun `invoke returns error when repository delete returns error`() = runTest {
        whenever(repository.deleteOutcome("OUT123")).thenReturn(Resource.Error("Failed to delete"))
    ...
    fun `invoke retries and succeeds after exceptions are thrown`() = runTest {
        var callCount = 0
        whenever(repository.deleteOutcome("OUT123")).thenAnswer {
            callCount++
            if (callCount < 2) {
        ...
        val result = deleteOutcomeUseCase(
    ...
    fun `invoke returns handleFailedSubmit after exhausting all retries on exception`() = runTest {
        whenever(repository.deleteOutcome("OUT123")).thenThrow(RuntimeException("Persistent error"))
        ...
        verify(repository, times(3)).deleteOutcome("OUT123")
    }
}
